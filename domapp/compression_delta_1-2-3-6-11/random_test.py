...
# Override the global constant values here.    
    ...
    if len(sys.argv)==5:
        ...
        print "Default file names are:"
        print "\t" + f1
        print "\t" + f2
        print "\t" + f3
        print "\t" + f4
        print
        if not raw_input("Use default file names? [y]").strip() \
               in ["", "y", "Y", "YES", "yes", "Yes"]:
            sys.exit()

    if len(sys.argv)==2 and sys.argv[1] in ['/n', '/N']:
        NUMBER_OF_RECORDS = 1
        CREATE_RANDOM_INPUT = False
...
##        print
##        print "=========================="
##        print "Run the vhdl simulation now with:"
##        print "\t Input file = ", f2
##        print "\tOutput file = ", f4
##        print "=========================="
##        raw_input(".....<return>")
...
# simulation is written to temp and discarded.
        ...
        if ans:
            print "=== Sucess ! ==="
            f.write("Run[" + str(run_number) + "] : success\n")
        else:
            import winsound
